...
public:
	...
	void setOrderMap(const std::map<int, SortingLayer&>& _sortingMap)
	{
		this->orderSortingMap = _sortingMap;
	}
	
	void setSpawnFunctionMap(const int id, const std::map<int, std::function<void(glm::vec2)>>& _spawnFunction)
	{
		this->gridLayers[id]->spawnFunctions = _spawnFunction;
	}
	
	void setTileFunctionMap(const int id, const std::map<int, std::function<Tile*()>>& _tileFunctions)
	{
		this->gridLayers[id]->tileFunctions = _tileFunctions;
	}
